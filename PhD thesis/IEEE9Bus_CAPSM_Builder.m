...
add_block('powerlib/Elements/Three-Phase Transformer (Two Windings)', ...
    [MODEL '/T1'], ...
    'NominalPower',   num2str(T1.S), ...
    'Freq',           num2str(f_nom), ...
    'Winding1',       mat2str([T1.V1, 0, T1.L_lv/2]), ...
    'Winding2',       mat2str([T1.V2, 0, T1.L_lv/2 * (T1.V2/T1.V1)^2]), ...
    'Rm',             '500', ...
    'Lm',             '500', ...
...
add_block('powerlib/Elements/Three-Phase Transformer (Two Windings)', ...
    [MODEL '/T2'], ...
    'NominalPower',   num2str(T2.S), ...
    'Freq',           num2str(f_nom), ...
    'Winding1',       mat2str([T2.V1, 0, T2.L_lv/2]), ...
    'Winding2',       mat2str([T2.V2, 0, T2.L_lv/2 * (T2.V2/T2.V1)^2]), ...
    'Rm',             '500', ...
    'Lm',             '500', ...
...
add_block('powerlib/Elements/Three-Phase Transformer (Two Windings)', ...
    [MODEL '/T3'], ...
    'NominalPower',   num2str(T3.S), ...
    'Freq',           num2str(f_nom), ...
    'Winding1',       mat2str([T3.V1, 0, T3.L_lv/2]), ...
    'Winding2',       mat2str([T3.V2, 0, T3.L_lv/2 * (T3.V2/T3.V1)^2]), ...
    'Rm',             '500', ...
    'Lm',             '500', ...
...
for k = 1:length(ln_fields)
    ...
    add_block('powerlib/Elements/Three-Phase PI Section Line', ...
        ...
        'R1',          num2str(ln.R), ...
        'L1',          num2str(ln.L), ...
        'C1',          num2str(ln.C_half*2), ...  % total C, block splits it
...
% --- SVC at Bus 5 (Phasor-type, ±100 MVAR range) ---
% Block: powerlib/FACTS/SVC (Phasor Type)
add_block('powerlib/FACTS/SVC (Phasor Type)', ...
    [MODEL '/SVC_Bus5'], ...
    'Pnom',       '100e6', ...    % Nominal reactive power [VAR]
    'Vref',       '1.0', ...      % Voltage reference [pu]
    'Xs',         '0.03', ...     % Slope (droop) reactance [pu]
    'Td',         '0.02', ...     % Delay time [s]
    'Kp',         '5', ...        % PI controller Kp
    'Ki',         '100', ...      % PI controller Ki
    'Position',   [950 230 1050 270]);

% --- STATCOM at Bus 8 (Phasor-type, ±50 MVAR) ---
% Faster than SVC — ideal for CAPSM System-1 fast reflex actions
add_block('powerlib/FACTS/STATCOM (Phasor Type)', ...
    [MODEL '/STATCOM_Bus8'], ...
    'Pnom',       '50e6', ...
    'Vref',       '1.0', ...
    'Xs',         '0.02', ...
    'Td',         '0.005', ...    % 5 ms — matches System-1 < 5 ms target
    'Kp',         '5', ...
    'Ki',         '200', ...
    'Position',   [950 640 1050 680]);

fprintf('[OK] FACTS: SVC (Bus5, ±100 MVAR), STATCOM (Bus8, ±50 MVAR) added\n');
...
set_param([ARBT '/Confidence_Estimator'], 'FunctionBody', ...
    sprintf(['function [mode, Ct, Nt] = fcn(state)\n'...
             '%% CAPSM Metacognitive Arbitration\n'...
             '%% Eq: alpha_t = 1 if Ct > tau_c AND Ut < tau_u; else 0\n'...
             'tau_c = 0.7; tau_u = 0.3;\n'...
             'Ct = 1 - std(state) / (std(state) + 0.01); %% confidence\n'...
             'Nt = norm(state - 0.5*ones(size(state))); %% novelty\n'...
             '%% 1 = System-1 (fast reflex), 0 = System-2 (deliberative)\n'...
             'mode = double((Ct > tau_c) && (Nt < tau_u));\n'...
             'end']));
...
set_param([SYS1 '/CNN_LSTM_Controller'], 'FunctionBody', ...
    sprintf(['function u_fast = fcn(pmu_data, mode)\n'...
             '%% System-1: CNN-LSTM Reflexive Controller\n'...
             '%% z(t) = LSTM(CNN(X_t))\n'...
             '%% u_fast = f_theta(z(t)) — maps to reactive injection\n'...
             '%% In deployment: replace with pretrained NN weights\n'...
             '%% Target response: < 5 ms (hardware) / < 0.2 ms (phasor sim)\n'...
             'if mode == 1  %% System-1 active\n'...
             '   %% Simplified: proportional reactive voltage support\n'...
             '   V_err  = mean(abs(pmu_data)) - 1.0; %% pu deviation\n'...
             '   Kprop  = 50e6; %% 50 MVAR/pu\n'...
             '   u_fast = -Kprop * V_err; %% reactive injection\n'...
             'else\n'...
             '   u_fast = 0; %% yield to System-2\n'...
             'end\n'...
             'end']));
...
set_param([SYS2 '/QIRL_Optimizer'], 'FunctionBody', ...
    sprintf(['function [dispatch, ev_pref] = fcn(state, soc, mode)\n'...
             '%% System-2: Quantum-Inspired Reinforcement Learning\n'...
             '%%\n'...
             '%% Policy: pi(a|s) represented as complex-valued amplitudes\n'...
             '%%   psi = sum_i alpha_i |a_i>\n'...
             '%%   P(a_i|s) = |alpha_i|^2\n'...
             '%%\n'...
             '%% Bellman update:\n'...
             '%%   Q(s,a) <- Q(s,a) + lr*(r + gamma*max_a Q(s,a) - Q(s,a))\n'...
             '%%\n'...
             '%% Tunneling: alpha_i <- alpha_i - lr*grad(L) + beta*T(alpha_i)\n'...
             '%% where T = tunneling operator (quantum annealing analog)\n'...
             '%%\n'...
             '%% ----- Simplified stub for co-simulation -----\n'...
             'if mode == 0  %% System-2 active\n'...
             '   %% Economic dispatch: minimize sum(c_i * P_i) s.t. balance\n'...
             '   P_load_total = 315e6; %% 125+90+100 MW\n'...
             '   c  = [0.11, 0.17, 0.21]; %% $/MWh cost coefficients\n'...
             '   Pg = [71.64e6, 163e6, 85e6]; %% MW initial dispatch\n'...
             '   %% QIRL adjustment toward optimal (simplified)\n'...
             '   delta_EV = (0.6 - soc) * 50e6; %% V2G: charge if SoC low\n'...
             '   dispatch  = Pg .* (1 + 0.01*(state(1)-1)); %% voltage droop\n'...
             '   ev_pref   = max(-50e6, min(50e6, delta_EV));\n'...
             'else\n'...
             '   dispatch = [71.64e6, 163e6, 85e6]; %% hold last dispatch\n'...
             '   ev_pref  = 0;\n'...
             'end\n'...
             'end']));
...
set_param([PINN '/PINN_Constraint_Check'], 'FunctionBody', ...
    sprintf(['function [u_safe, viol] = fcn(u_prop, state)\n'...
             '%% PINN Safety Layer\n'...
             '%% Constraint residuals (from AC power flow physics):\n'...
             '%%   L_pf = ||F(V,theta,P,Q)||^2  (power balance equations)\n'...
             '%%   L_lim = sum max(0, V-Vmax)^2 + max(0, Vmin-V)^2\n'...
             '%%            + sum max(0, I-Imax)^2 + max(0, SoC-1)^2\n'...
             '%% Target: >99.8%% constraint compliance (PhD proposal Table 7)\n'...
             '%%\n'...
             '%% Voltage limits [pu]\n'...
             'V_min = 0.90; V_max = 1.10;\n'...
             '%% Current thermal limits [A at 230 kV]\n'...
             'I_max_pu = 1.05;\n'...
             '%% State interpretation: state = [V_bus4..9, I_line45..89]\n'...
             'V = state(1:6); I = state(7:12);\n'...
             '%% Constraint violation check\n'...
             'viol_V = any(V < V_min) || any(V > V_max);\n'...
             'viol_I = any(abs(I) > I_max_pu);\n'...
             'viol = double(viol_V || viol_I);\n'...
             '%% Project control action back to feasible region\n'...
             'if viol\n'...
             '   %% Penalty projection: reduce action magnitude\n'...
             '   lambda_pf = 10; lambda_lim = 50;\n'...
             '   V_err = max(0, V - V_max) - max(0, V_min - V);\n'...
             '   correction = -lambda_lim * sum(V_err);\n'...
             '   u_safe = u_prop + correction;\n'...
             'else\n'...
             '   u_safe = u_prop;\n'...
             'end\n'...
             'end']));
...
set_param([ATCK '/Attack_Injector'], 'FunctionBody', ...
    sprintf(['function pmu_out = fcn(pmu_in, t)\n'...
             '%% Cyber-Attack Injector for CAPSM Resilience Testing\n'...
             '%% Attack schedule (seconds):\n'...
             '%%   t=[2,5]:   FDI — additive Gaussian noise on Bus 5 PMU\n'...
             '%%   t=[8,8.5]: DoS — zero out all PMU signals\n'...
             '%%   t=[12,20]: Communication delay (1.5s, modeled as hold)\n'...
             'pmu_out = pmu_in;\n'...
             'if t >= 2 && t <= 5\n'...
             '   %% False Data Injection: Gaussian noise, SNR=20 dB\n'...
             '   sigma  = 0.05 * max(abs(pmu_in));\n'...
             '   pmu_out = pmu_in + sigma * randn(size(pmu_in));\n'...
             'elseif t >= 8 && t <= 8.5\n'...
             '   %% Denial of Service: zero all measurements\n'...
             '   pmu_out = zeros(size(pmu_in));\n'...
             'end\n'...
             'end']));
...
try
    % Gen1 → T1 primary
    add_line(MODEL, 'Gen1/LConn1',  'T1/LConn1',  'autorouting','on');
    % Gen2 → T2 primary
    add_line(MODEL, 'Gen2/LConn1',  'T2/LConn1',  'autorouting','on');
    % Gen3 → T3 primary
    add_line(MODEL, 'Gen3/LConn1',  'T3/LConn1',  'autorouting','on');

    % T1 secondary → Line_4_5 (Bus 4 node)
    add_line(MODEL, 'T1/RConn1',    'Line_4_5/LConn1', 'autorouting','on');
    add_line(MODEL, 'T1/RConn1',    'Line_4_6/LConn1', 'autorouting','on');

    % Line_4_5 → Load_Bus5 (Bus 5 node)
    add_line(MODEL, 'Line_4_5/RConn1', 'Line_5_7/LConn1', 'autorouting','on');
    add_line(MODEL, 'Line_4_5/RConn1', 'Load_Bus5/LConn1','autorouting','on');

    % Line_4_6 → Load_Bus6 (Bus 6 node)
    add_line(MODEL, 'Line_4_6/RConn1', 'Line_6_9/LConn1', 'autorouting','on');
    add_line(MODEL, 'Line_4_6/RConn1', 'Load_Bus6/LConn1','autorouting','on');

    % Line_5_7 → Bus7 (T2 HV side)
    add_line(MODEL, 'Line_5_7/RConn1', 'T2/RConn1',       'autorouting','on');
    add_line(MODEL, 'Line_5_7/RConn1', 'Line_7_8/LConn1', 'autorouting','on');

    % Line_7_8 → Bus8
    add_line(MODEL, 'Line_7_8/RConn1', 'Line_8_9/LConn1', 'autorouting','on');
    add_line(MODEL, 'Line_7_8/RConn1', 'Load_Bus8/LConn1','autorouting','on');

    % Line_6_9 → Bus9 (T3 HV side)
    add_line(MODEL, 'Line_6_9/RConn1', 'T3/RConn1',       'autorouting','on');
    add_line(MODEL, 'Line_6_9/RConn1', 'Line_8_9/RConn1', 'autorouting','on');

    fprintf('[OK] All power network connections established\n');
catch ME
    fprintf('[WARN] Autorouting connections: %s\n', ME.message);
    fprintf('       → Manual wiring may be needed for composite ports in your version\n');
    fprintf('       → Open model and connect blocks manually using the topology above\n');
end
...
% =========================================================================
try
    add_line(MODEL, 'Load_Bus5/RConn1', 'Gnd_Bus5/LConn1', 'autorouting','on');
    add_line(MODEL, 'Load_Bus6/RConn1', 'Gnd_Bus6/LConn1', 'autorouting','on');
    add_line(MODEL, 'Load_Bus8/RConn1', 'Gnd_Bus8/LConn1', 'autorouting','on');
    fprintf('[OK] Ground references connected\n');
catch ME
    fprintf('[WARN] Ground connections: %s\n', ME.message);
end
